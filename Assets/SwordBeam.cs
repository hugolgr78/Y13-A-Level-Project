...
public class SwordBeam : MonoBehaviour
{
	...
	void OnTriggerEnter2D (Collider2D hitInfo)
	{
        ...
        {
            hitInfo.GetComponent<Enemy>().TakeDamage(5);
            if(hitInfo.GetComponent<Enemy>().EnemyIsDebuffed)
            {
                hitInfo.GetComponent<Enemy>().ResetDebuffTimer();
            } else {
                hitInfo.GetComponent<Enemy>().DebuffEnemy();
            }
